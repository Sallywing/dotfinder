function err = TrainingError(Thrsh)
    
    global group Colo Dots DotIdx Settings
    IntenThrsh = Thrsh(1);
    NoiseThrsh = Thrsh(2);
    ...
    elseif sum(isnan(ColoMinDists)) > (length(ColoMinDists)/2)
        ...
        [C,err,P,logp,coeff] = classify([min(ColoMinDists)-1:0.001:max(ColoMinDists)+1]',ColoMinDists(~isnan(ColoMinDists))',group(~isnan(ColoMinDists)));
    end
...
if exist([TPN 'find\SG.mat']) %7/28/2010 HO
...
if SGOptions.EdgeDotCut;
    ...
    if EdgeDotCutRedoFlag == 1; %if-end loop added to remove the repetition 1/4/2011 HO
        ...
        EdgeDotInd = find(EdgeVoxIDMap>0);
        EdgeDotIDs = EdgeVoxIDMap(EdgeDotInd);
        EdgeDotIDs = unique(EdgeDotIDs);
...
if SGOptions.DoubleZDotCut;
    if exist([TPN 'data\NonDoubleZDots.mat']) %if-end loop added to remove the repetition 1/4/2011 HO
    ...
    if DoubleZDotCutRedoFlag == 1; %if-end loop added to remove the repetition 1/4/2011 HO
        ...
        save([TPN 'data\NonDoubleZDots.mat'],'NonDoubleZDots') %saving changed to Colo 7/28/2010 HO
        pass = pass & NonDoubleZDots; % Exclude double Z dots
        
    else
        load([TPN 'data\NonDoubleZDots.mat']);
        pass = pass & NonDoubleZDots; % Exclude double Z dots
    end
end
...
passF=pass2;
if gP.forceCorrect
    if exist([TPN 'find\yes.tif'])| exist([TPN 'find\YesNo.mat'])
        passF(Hit)=1;
    end
    passF(Miss)=0;
end
...
if exist([TPN 'find\yes3Dforce.tif']) %if-end loop added 1/29/2011 HO
    YesIM = zeros(Dots.ImSize);
    for i = 1:Dots.ImSize(3)
        YesIM(:,:,i)=imread([TPN 'find\yes3Dforce.tif'], i);
    end
    VoxIDMap = zeros(Dots.ImSize);
    for i=1:Dots.Num
        VoxIDMap(Dots.Vox(i).Ind)=i;
...
if isfield(SG,'passI')
    ...
    imwrite(ImarCrit,[TPN 'find\ImarCrit.tif'],'Compression','none')
    imwrite(MaxC,[TPN 'find\ImarMaxCids.tif'],'Compression','none')

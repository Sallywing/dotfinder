...
%WQY 2/8/2016 added mask processing for BC axonal terminals
...
if exist([TPN 'Colo.mat']);
    ...
    Colo = HOtif2mat('Load Coloc 1'); %Or, load tif file into mat if the fish is not dot, like BC axons and their markers
    save([TPN 'Colo.mat'], 'Colo');
    coloc1DotsNum = length(Dots.Pos(:,1));
    Dots.Coloc1Pos = Dots.Pos;
    ColoMaskFlag = input('Do you need to apply a mask on Coloc 1? Y:1, N:0 \n');
    if ColoMaskFlag
        ColoMask = HOtif2mat('Load Mask for Coloc 1');
        save([TPN 'ColoMask.mat'], 'ColoMask');
        Dots.Coloc1Pos = dotMask(ColoMask, Dots);
        coloc1DotsNum = length(Dots.Coloc1Pos(:,1));
    end
    coloc1DotsNum
...
if exist([TPN 'Colo2.mat']);
    ...
    Colo2 = HOtif2mat('Load Coloc 2'); %Or, load tif file into mat if the fish is not dot, like BC axons and their markers
    save([TPN 'Colo2.mat'], 'Colo2');
    coloc2DotsNum = length(Dots.Pos(:,1));
    Dots.Coloc2Pos = Dots.Pos;
    ColoMaskFlag2 = input('Do you need to apply a mask on Coloc 2? Y:1, N:0 \n');
    if ColoMaskFlag2
        ColoMask2 = HOtif2mat('Load Mask for Coloc 2');
        save([TPN 'ColoMask2.mat'], 'ColoMask2');
        Dots.Coloc2Pos = dotMask(ColoMask2, Dots);
        coloc2DotsNum = length(Dots.Coloc2Pos(:,1));
    end
    coloc2DotsNum
    save([TPN 'Dots.mat'], 'Dots');
end
...
if exist([TPN 'ColocManual.mat']) %load the one in the middle of analysis
    ...
    elseif ManualAllDotsFlag == 0;
        ...
        while AddAnotherDendFlag
            AddAnotherDendFlag = input('Want to sample at least 100 dots. Type 1 for adding another dendrite, 0 if you are satisfied with the dot number, 3 if you want to sample equally from center to periphery. \n');
    ...
    elseif ManualAllDotsFlag == 3; %choose some of dots equally from the center to peripheral
    ...
    else %start with masked dots
        NumDotsManuallyAnalyzed = input('How many dots do you want to sample? \n');
        load([TPN 'SkelFiner.mat']);
        GroupedNum = [];
        SomaVoxPosXYZ = ceil([Skel.FilStats.SomaPtXYZ(:,1)/Settings.ImInfo.xyum, Skel.FilStats.SomaPtXYZ(:,2)/Settings.ImInfo.xyum, Skel.FilStats.SomaPtXYZ(:,3)/Settings.ImInfo.zum]);
        DistDot2Soma = sqrt((Dots.Pos(:,1)-SomaVoxPosXYZ(1)).^2 + (Dots.Pos(:,2)-SomaVoxPosXYZ(2)).^2 + (Dots.Pos(:,2)-SomaVoxPosXYZ(3)).^2);
        if NumDotsManuallyAnalyzed > size(union(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),1)
            [Cm, idx1, idx2] = intersect(Dots.Pos,union(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'));
            ManualColocAnalyzingFlag(idx1) = 1; % must contain union of coloc1,coloc2
            [Cm, idx] = setdiff(Dots.Pos, union(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'), 'rows');
            % assign dots to 10 bins according to their Dist2Soma
            [m,n] = histc(DistDot2Soma(idx),min(DistDot2Soma(idx)):range(DistDot2Soma(idx))/9:max(DistDot2Soma(idx)));
            NumSampledDots = floor(m/sum(m)*(NumDotsManuallyAnalyzed-size(union(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),1)));
            for i = 1:length(m)
                GroupedNum = cat(1, GroupedNum, randsample(find(n == i), NumSampledDots(i)));
            end
            ManualColocAnalyzingFlag(idx(GroupedNum)) = 1;
        elseif NumDotsManuallyAnalyzed > size(intersect(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),1)
            [Cm, idx1, idx2] = intersect(Dots.Pos,intersect(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),'rows');
            ManualColocAnalyzingFlag(idx1) = 1; % must contain intersect of coloc1,coloc2
            [Ct] = setdiff(union(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),intersect(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),'rows');
            [Cm, idx, idx2] = intersect(Dots.Pos, Ct, 'rows');
            % assign dots to 10 bins according to their Dist2Soma
            [m,n] = histc(DistDot2Soma(idx),min(DistDot2Soma(idx)):range(DistDot2Soma(idx))/9:max(DistDot2Soma(idx)));
            NumSampledDots = floor(m/sum(m)*(NumDotsManuallyAnalyzed-size(intersect(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),1)));
            for i = 1:length(m)
                GroupedNum = cat(1, GroupedNum, randsample(find(n == i), NumSampledDots(i)));
            end
            ManualColocAnalyzingFlag(idx(GroupedNum)) = 1;
        elseif NumDotsManuallyAnalyzed < size(intersect(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),1)
            [Cm, idx, idx2] = intersect(Dots.Pos,intersect(Dots.Coloc1Pos,Dots.Coloc2Pos,'rows'),'rows');
            % assign dots to 10 bins according to their Dist2Soma
            [m,n] = histc(DistDot2Soma(idx),min(DistDot2Soma(idx)):range(DistDot2Soma(idx))/9:max(DistDot2Soma(idx)));
            NumSampledDots = floor(m/sum(m)*(NumDotsManuallyAnalyzed));
            for i = 1:length(m)
...
%add codes for coloc masked dots
[Ct, idx, ColocPos1Idx] = intersect(Dots.Coloc1Pos, Dots.Pos, 'rows');
[Ct, idx, ColocPos2Idx] = intersect(Dots.Coloc2Pos, Dots.Pos, 'rows');
[ColocSamPos1Idx, idx, idx1] = intersect(ColocPos1Idx, ColocManual.ListDotIDsManuallyColocAnalyzed);
[ColocSamPos2Idx, idx, idx2] = intersect(ColocPos2Idx, ColocManual.ListDotIDsManuallyColocAnalyzed);

%add stats so that you can remember ColocFlag of 1 is coloc, etc.
ColocManual.NumDotsColocFish1 = length(find(ColocManual.ColocFlagFish1(idx1) == 1));
ColocManual.NumDotsNonColocFish1 = length(find(ColocManual.ColocFlagFish1(idx1) == 2));
ColocManual.NumDotsColocFish2 = length(find(ColocManual.ColocFlagFish2(idx2) == 1));
ColocManual.NumDotsNonColocFish2 = length(find(ColocManual.ColocFlagFish2(idx2) == 2));
ColocManual.NumFalseDots1 = length(find(ColocManual.ColocFlagFish1(idx1) == 3));
ColocManual.NumFalseDots2 = length(find(ColocManual.ColocFlagFish1(idx2) == 3));
...
ColocManual.FalseDotRate1 = ColocManual.NumFalseDots1/(ColocManual.NumDotsColocFish1+ColocManual.NumDotsNonColocFish1+ColocManual.NumFalseDots1);
ColocManual.FalseDotRate2 = ColocManual.NumFalseDots2/(ColocManual.NumDotsColocFish2+ColocManual.NumDotsNonColocFish2+ColocManual.NumFalseDots2);
ColocManual.ColocRateFish1InclugingFalshDots = ColocManual.NumDotsColocFish1/(ColocManual.NumDotsColocFish1+ColocManual.NumDotsNonColocFish1+ColocManual.NumFalseDots1);
ColocManual.ColocRateFish2InclugingFalshDots = ColocManual.NumDotsColocFish2/(ColocManual.NumDotsColocFish2+ColocManual.NumDotsNonColocFish2+ColocManual.NumFalseDots2);

ColocManual.TotalNumDotsManuallyColocAnalyzed1 = length(idx1);
ColocManual.TotalNumDotsManuallyColocAnalyzed2 = length(idx2);
ColocManual.ListDotIDsManuallyColocAnalyzed1 = ColocSamPos1Idx;
ColocManual.ListDotIDsManuallyColocAnalyzed2 = ColocSamPos2Idx;
save([TPN 'ColocManual.mat'], 'ColocManual'); 
...
%     answer = inputdlg(prompt,dlg_title,num_lines,def);
    MaxColoThreshAccuracy = [];
    MaxColo2ThreshAccuracy = [];
    MostAccurateColoDistanceThresh = [];
    MostAccurateColo2DistanceThresh = [];
    tic;
    IntenThrsh = 10:5:100;
    NoiseAcc.MaxNoiseRegions = 0:5:80;
    for it = 1:length(IntenThrsh)
    ColoMaskMinIntensity = IntenThrsh(it);
    Colo2MaskMinIntensity = IntenThrsh(it);
    BestMinIntensityNotFound = 1;

    NoiseAcc.ColoAccuracies = zeros(1,numel(NoiseAcc.MaxNoiseRegions));
    NoiseAcc.Colo2Accuracies = zeros(1,numel(NoiseAcc.MaxNoiseRegions));
...
%         tic % start timer to test for loop
        for J = 1:(numel(NoiseAcc.MaxNoiseRegions))
            it,J
...
%             end
            ...
            ColoMinDists = zeros(1,ColocManual.TotalNumDotsManuallyColocAnalyzed1);
            Colo2MinDists = zeros(1,ColocManual.TotalNumDotsManuallyColocAnalyzed2);
            CutNumVox = [20, 20, 20];
            idx1 = 1; idx2 = 1;
            for DotNum = ColocManual.ListDotIDsManuallyColocAnalyzed
                if ismember(DotNum, ColocManual.ListDotIDsManuallyColocAnalyzed1) 
                    ...
                    if ~isempty(find(ColoMaskCut == 1))
                        ColoMinDists(idx1) = min(ColoDistMask(find(ColoMaskCut == 1))); % Find the point in mask that is closest to dot
                    else
                        ColoMinDists(idx1) = NaN;
                    end
                    idx1 = idx1 + 1;
                end
                if ismember(DotNum, ColocManual.ListDotIDsManuallyColocAnalyzed2)
                    ...
                    if ~isempty(find(Colo2MaskCut == 1))
                        Colo2MinDists(idx2) = min(Colo2DistMask(find(Colo2MaskCut == 1))); % Find the point in mask that is closest to dot
                    else
                        Colo2MinDists(idx2) = NaN;
                    end
                    idx2 = idx2 + 1;
                end
...
%             MostAccurateColo2DistanceThresh = Colo2Thresholds(Colo2Idx);
            idx = (ColocManual.ColocFlagFish1 == 1 | ColocManual.ColocFlagFish1 == 2) & ~isnan(ColoMinDists);
            if all(ColoMinDists(idx) == 0)
                MaxColoThreshAccuracy(it,J) = 0.5;
                MostAccurateColoDistanceThresh(it,J) = 0;
            else
                y=ColocManual.ColocFlagFish1(idx);
                y(y==2)=0;
                [MaxColoThreshAccuracy(it,J),MostAccurateColoDistanceThresh(it,J)] ...
                    = GLMmaxthrsh(ColoMinDists(idx)',double(y),'binomial','logit',0.5);
            end
            idx = (ColocManual.ColocFlagFish2 == 1 | ColocManual.ColocFlagFish2 == 2) & ~isnan(Colo2MinDists);
            if all(Colo2MinDists(idx) == 0)
                MaxColo2ThreshAccuracy(it,J) = 0.5;
                MostAccurateColo2DistanceThresh(it,J) = 0;
            else
                y=ColocManual.ColocFlagFish2(idx);
                y(y==2)=0;
                [MaxColo2ThreshAccuracy(it,J),MostAccurateColo2DistanceThresh(it,J)] ...
                    = GLMmaxthrsh(Colo2MinDists(idx)',double(y),'binomial','logit',0.5);
            end
            
%             if J ~= (numel(NoiseAcc.MaxNoiseRegions) + 1) %If we are currently testing accuracies
%                 NoiseAcc.ColoAccuracies(it,J) = MaxColoThreshAccuracy;
%                 NoiseAcc.Colo2Accuracies(it,J) = MaxColo2ThreshAccuracy;
%             end
...
%         end
    ...
    toc
     ...
     [C,I]=max(MaxColoThreshAccuracy(:));
     [C2,I2]=max(MaxColo2ThreshAccuracy(:));
     ColocManual.ColoDisThrsh = MostAccurateColoDistanceThresh(I);
     ColocManual.Colo2DisThrsh = MostAccurateColo2DistanceThresh(I2);
     ColocManual.ColoThreshAccuracy = C;
     ColocManual.Colo2ThreshAccuracy = C2;
     ColocManual.ColoThresh = [IntenThrsh(rem(I-1,length(IntenThrsh))+1), NoiseAcc.MaxNoiseRegions(ceil(I/length(IntenThrsh)))];
     ColocManual.Colo2Thresh = [IntenThrsh(rem(I2-1,length(IntenThrsh))+1), NoiseAcc.MaxNoiseRegions(ceil(I2/length(IntenThrsh)))];
...
%     ColocManual.Colo2MinDists = Colo2MinDists;
    ...
    ColocAuto.ColocFlagFish1 = zeros([1,ColocAuto.TotalNumDotsAutoColocAnalyzed], 'uint8')+2;
    ColocAuto.ColocFlagFish2 = zeros([1,ColocAuto.TotalNumDotsAutoColocAnalyzed], 'uint8')+2;
    ...
    ColocAuto.ColoMinDists = findColoDists(ColocManual.ColoThresh, Colo, Dots, ColocAuto.ListDotIDsAutoColocAnalyzed, Settings);
    ColocAuto.Colo2MinDists = findColoDists(ColocManual.Colo2Thresh, Colo2, Dots, ColocAuto.ListDotIDsAutoColocAnalyzed, Settings);
    ColocAuto.ColocFlagFish1(ColocAuto.ColoMinDists < ColocManual.ColoDisThrsh) = 1;
    ColocAuto.ColocFlagFish2(ColocAuto.Colo2MinDists < ColocManual.Colo2DisThrsh) = 1;
...
%    ColocAuto.NoiseAcc = NoiseAcc;
    ...
    ColocAuto.ColoMaskMinIntensity = ColocManual.ColoThresh;
    ColocAuto.MaxColoThreshAccuracy = ColocManual.ColoThreshAccuracy;
    ColocAuto.MostAccurateColoDistanceThresh = ColocManual.ColoDisThrsh;
    ColocAuto.NumDotsColocFish1 = sum(ColocAuto.ColocFlagFish1(ColocPos1Idx) == 1);
    ColocAuto.NumDotsNonColocFish1 = sum(ColocAuto.ColocFlagFish1(ColocPos1Idx) == 2);
    ColocAuto.ColocRateFish1 = ColocAuto.NumDotsColocFish1/length(ColocPos1Idx);
...
%     ColocAuto.ColocRateFish1 = ColocAuto.NumDotsColocFish1 / double(ColocAuto.TotalNumDotsAutoColocAnalyzed);
    ...
    ColocAuto.Colo2MaskMinIntensity = ColocManual.Colo2Thresh;
    ColocAuto.MaxColo2DistanceThreshAccuracy = ColocManual.Colo2ThreshAccuracy;
    ColocAuto.MostAccurateColo2DistanceThresh = ColocManual.Colo2DisThrsh;
    ColocAuto.NumDotsColocFish2 = sum(ColocAuto.ColocFlagFish2(ColocPos2Idx) == 1);
    ColocAuto.NumDotsNonColocFish2 = sum(ColocAuto.ColocFlagFish2(ColocPos2Idx) == 2);
    ColocAuto.ColocRateFish2 = ColocAuto.NumDotsColocFish2/length(ColocPos2Idx);
...
%     ColocAuto.ColocRateFish2 = ColocAuto.NumDotsColocFish2 / double(ColocAuto.TotalNumDotsAutoColocAnalyzed);
    ...
    ColocTotal.NumDotsColocFish1 = sum(ColocTotal.ColocFlagFish1(ColocPos1Idx) == 1);
    ColocTotal.NumDotsNonColocFish1 = sum(ColocTotal.ColocFlagFish1(ColocPos1Idx) == 2);
    ColocTotal.ColocRateFish1 = ColocTotal.NumDotsColocFish1/length(ColocPos1Idx);
    ...
    ColocTotal.NumDotsColocFish2 = sum(ColocTotal.ColocFlagFish2(ColocPos2Idx) == 1);
    ColocTotal.NumDotsNonColocFish2 = sum(ColocTotal.ColocFlagFish2(ColocPos2Idx) == 2);
    ColocTotal.ColocRateFish2 = ColocTotal.NumDotsColocFish2/length(ColocPos2Idx);
